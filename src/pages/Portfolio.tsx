...
export default function Portfolio({ isHome = false }: { isHome?: boolean }) {
  ...
  return (
    <div className="w-full max-w-7xl mx-auto px-4 py-16 md:py-20 flex flex-col items-center">
      <div className="mb-12 md:mb-16 inline-flex items-center justify-center px-6 py-2 rounded-full glow-gradient-border bg-black">
        ...
        {displayProjects.map((project, index) => (
          ...
          >
            <Link to={`/portfolio/${project.id}`} className="group w-full flex flex-col items-center">
              <div className="w-full aspect-[4/3] rounded-[1.5rem] md:rounded-[2rem] p-1 glow-gradient-border overflow-hidden mb-4 transition-transform duration-300 group-hover:scale-[1.02]">
              ...
              <h3 className="text-lg md:text-xl font-semibold text-white tracking-tight group-hover:text-purple-400 transition-colors duration-300">
                {project.title}
              </h3>
            </Link>
          </motion.div>
